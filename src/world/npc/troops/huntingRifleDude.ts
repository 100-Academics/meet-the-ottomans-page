...
export class HuntingRifleDude extends npc {
	...
	private readonly rangedCooldownSeconds = 0.5;
	private readonly rangedProjectileSpeed = 70;

	private lastRangedAttackTime = -Infinity;
	private meleeWindupPending = false;

	constructor(id: number, modelEntity: Entity = new Entity("hunting rifle dude")) {
	...
	private fireProjectileAt(
		...
		const projectile = new Entity("hunting rifle bullet");
		projectile.setPosition(origin);
		projectile.lookAt(origin.clone().add(dir));

		const tracer = new Entity("hunting rifle bullet tracer");
