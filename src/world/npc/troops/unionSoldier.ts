...
export class UnionSoldier extends npc {
	...
	constructor(id: number, modelEntity: Entity = new Entity("union soldier")) {
	...
	private fireProjectileAt(
		...
		const projectile = new Entity("union bullet");
		projectile.setPosition(origin);
		projectile.lookAt(origin.clone().add(dir));

		const tracer = new Entity("union bullet tracer");
